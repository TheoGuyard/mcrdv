...
Sweeping is deliberately not something `run.py` knows about. An entry of
`experiments/sweeps.yaml` names the axes to explore; `script.py make` expands
them from the experiment's `run.yaml` with the helpers below and writes them to
a disposable directory, and the runner then treats each one as an ordinary
...
def grid(
    ...
    `grid(base, **{"solver.args.schedule.args.grid_size": [8, 32],
    "seed": range(3)})`.
