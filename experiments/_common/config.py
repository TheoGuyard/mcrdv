...
transfer layer is, or that a solver's `preset` means anything -- that belongs to
...
def derive(base: Mapping[str, Any], **overrides: Any) -> Dict[str, Any]:
    ...
    `**{"solver.args.sequence.args.crossover": "ox"}`; plain names work as
    usual.

    The values are copied as well as the base, so that what comes out owns
    every part of itself. A sweep writes an instance once and refers to it from
    a dozen places through a YAML anchor -- one object, shared -- and assigning
    it as it stands would leave each configuration holding that same object,
    where a later override reaching into it would edit the ones already made.
    """
    out = copy.deepcopy(dict(base))
    for path, value in overrides.items():
        put(out, path, copy.deepcopy(value))
    return out
