...
So selection is data and rendering is code. A figure named in the YAML maps to a
function in the experiment's `plot.py`, which receives the rows already selected
and reduced.
"""
...
def main(
    ...
    figures : mapping
        Figure name -> drawing function, one per entry in the configuration.
    """
    ...
    source = args.results or (here / str(spec.get("source", "results")))
    if not Path(source).exists():
        raise SystemExit(f"{source} missing -- run the experiment first")

    records = list(record.load(Path(source)))
    if not records:
        raise SystemExit(f"no records in {source}")
    rows = [r.row() for r in records]
    print(f"{here.name}: {len(records)} records from {source}")

    written: List[Path] = []
    drawn: List[str] = []
    for entry in spec.get("figures", []):
        name = str(entry.get("name", ""))
        if args.only and name != args.only:
            continue
        if name not in figures:
            print(f"  skipping '{name}': no drawing function registered")
            continue
        data = prepare(rows, records, entry)
        if not data.rows:
            print(f"  skipping '{name}': nothing matched its selection")
            continue
        drawn.append(name)
        paths = draw(name, figures[name], data, args.out, args.save)
        written += paths
        if paths:
            # The files are named by timestamp, so say which figure they are.
            print(f"  {name} -> {paths[0].stem}")
