...
fn test_solver_deterministic_with_same_seed() {
    ...
    let mut solver1 = Solver::preset(0);
    let (sol1, _) = solver1.solve(&problem, &oracle);

    let mut solver2 = Solver::preset(0);
    let (sol2, _) = solver2.solve(&problem, &oracle);
...
fn test_solver_different_seeds_may_differ() {
    ...
    let mut solver1 = Solver::preset(2);
    let (sol1, _) = solver1.solve(&problem, &oracle);

    let mut solver2 = Solver::preset(2);
    let (sol2, _) = solver2.solve(&problem, &oracle);
...
fn test_solver_max_chasers_respected() {
    ...
    let oracle = default_oracle();
    let mut solver = Solver::preset(0);
    solver.params.limit_time = 5.0;
    let (sol, _) = solver.solve(&problem, &oracle);

    assert_eq!(
        sol.sequences.len(),
        3,
        "Should have exactly max_chasers sequences"
    );
...
fn test_solver_time_limit_respected() {
    ...
    let time_limit = 2.0;
    let mut solver = Solver::preset(6);
    solver.params.limit_time = time_limit;
